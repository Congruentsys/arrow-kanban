...
impl ItemType {
    ...
    pub fn from_str_loose(s: &str) -> Option<ItemType> {
        match s.to_lowercase().as_str() {
            ...
            "campaign" | "camp" | "ca" => Some(ItemType::Campaign),
...
mod tests {
    ...
    fn test_campaign_is_a_dev_container_type() {
        ...
        assert_eq!(ItemType::Campaign.board(), "development");
        assert!(ItemType::DEV.contains(&ItemType::Campaign));
        for alias in ["campaign", "Campaign", "CAMPAIGN", "camp", "ca", "CA"] {
            assert_eq!(
                ItemType::from_str_loose(alias),
                Some(ItemType::Campaign),
                "alias {alias:?} should parse to Campaign"
            );
        }
    }

    #[test]
    fn test_no_prefix_collisions_across_all_types() {
        // Adding CA must not collide with any existing current-or-legacy prefix.
        let mut seen = std::collections::HashSet::new();
        for &ty in ItemType::DEV.iter().chain(ItemType::RESEARCH.iter()) {
            for p in ty.all_prefixes() {
                assert!(
                    seen.insert(p),
                    "prefix {p:?} is used by more than one item type"
                );
            }
        }
    }
