...
//! or SHACL shapes.
//!
//! Override hierarchy: project (.arrow-kanban/themes/) → built-in (compiled).
...
impl Theme {
    ...
    /// Load the nautical theme (the default).
    ...
    pub fn load(name: &str, project_root: Option<&Path>) -> Option<Self> {
        ...
        if let Some(root) = project_root {
            let path = root.join(format!(".arrow-kanban/themes/{name}.ttl"));
    ...
    pub fn available_themes(project_root: Option<&Path>) -> Vec<String> {
        ...
        if let Some(root) = project_root {
            let themes_dir = root.join(".arrow-kanban/themes");
...
mod tests {
    ...
    fn test_project_override() {
        ...
        let themes_dir = dir.path().join(".arrow-kanban/themes");
