...
//! Migration — parse markdown files (with turtle frontmatter) into Arrow tables.
//!
...
//! Usage:
//! ```ignore
//! let config = ConfigFile::from_path(&Path::new(".arrow-kanban/config.yaml"))?;
...
/// matching the behavior for legacy items.
...
"#,
    ...
    fn test_duplicate_id_detection() {
        ...
        let config_dir = dir.path().join(".arrow-kanban");
