...
//! Query engine — NL filter extraction, SPARQL subset, and hybrid text search.
//!
//! Supports four modes:
...
//! 4. **Hybrid** — NL extraction + text/substring ranking on remaining terms
...
/// A ranked query result combining structured filters and text-match scores.
#[derive(Debug, Clone)]
...
/// Execute a hybrid query: NL decomposition + structured filters + text ranking.
///
/// 1. Parse NL query to extract structured filters + remaining text
/// 2. Apply structured filters to narrow candidates
/// 3. Score remaining candidates by title text match
/// 4. Return results sorted by score
pub fn hybrid_query(batches: &[RecordBatch], query_str: &str, top_k: usize) -> Vec<RankedResult> {
...
mod tests {
    ...
    fn test_text_matches() {
        ...
        assert!(!text_matches("Arrow-Kanban Engine", "database"));
    }
    ...
    fn test_hybrid_query_structured_only() {
        ...
        let results = hybrid_query(store.items_batches(), "backlog expeditions", 20);
    ...
    fn test_hybrid_query_with_text() {
        ...
        let results = hybrid_query(store.items_batches(), "arrow kanban", 20);
    ...
    fn test_hybrid_query_id_pattern() {
        ...
        let results = hybrid_query(store.items_batches(), "EX-1300", 20);
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].id, "EX-1300");
