...
//! Configuration lives in `.arrow-kanban/hooks.yaml`.
//!
//! # Supported events
//!
...
impl HookEngine {
    ...
    /// Load hooks from `.arrow-kanban/hooks.yaml`.
    pub fn load(root: &Path) -> Self {
        let hooks_path = root.join(".arrow-kanban/hooks.yaml");
        ...
        let action = match action_str {
            ...
            "log" => {
                let target = entry
                    .get("target")
                    .and_then(|v| v.as_str())
                    .unwrap_or(".arrow-kanban/hooks.log")
                    .to_string();
                HookAction::Log {
