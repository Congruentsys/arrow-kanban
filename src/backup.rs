...
//! ```
//!
//! # Backup Config (in .arrow-kanban/config.yaml)
//!
//! ```yaml
//! backup:
...
/// Backup section in .arrow-kanban/config.yaml.
...
impl SnapshotMetadata {
    ...
    fn last_commit_id() -> Option<String> {
        let commits_path = Path::new(".arrow-kanban/_commits.json");
...
/// Files to snapshot from .arrow-kanban/
...
pub fn create_snapshot(config: &BackupConfig, root: &Path) -> Result<PathBuf> {
    ...
    let files_to_copy: Vec<&str> = SNAPSHOT_FILES.to_vec();
...
/// .arrow-kanban/, overwriting existing files. Requires --force to confirm.
