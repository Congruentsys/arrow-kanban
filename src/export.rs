...
pub fn export_item_json_full(
    ...
    use arrow::array::{BooleanArray, TimestampMillisecondArray};

    // Start from the item's own fields — export_json returns a 1-element array.
    let mut root: serde_json::Value =
        serde_json::from_str::<serde_json::Value>(&export_json(std::slice::from_ref(item)))
            .ok()
    ...
    // Status history: every run row whose item_id matches, oldest-first (runs are
    // appended in transition order), as {from, to, at, by, forced, reason}.
    ...
    for batch in runs {
        ...
        let reason = batch
            ...
            .as_any()
            .downcast_ref::<StringArray>();
        let (Some(ids), Some(from), Some(to), Some(at), Some(by), Some(forced), Some(reason)) =
            (ids, from, to, at, by, forced, reason)
        else {
            continue; // non-canonical runs batch — skip, never fabricate a transition
        };
        for i in 0..batch.num_rows() {
            if ids.value(i) != item_id {
                continue;
            }
            history.push(serde_json::json!({
                "from": opt_str((!from.is_null(i)).then(|| from.value(i))),
                "to": to.value(i),
                "at": at.value(i),
                "by": opt_str((!by.is_null(i)).then(|| by.value(i))),
                "forced": forced.value(i),
                "reason": opt_str((!reason.is_null(i)).then(|| reason.value(i))),
            }));
        }
    }
    ...
    // Emit a 1-element ARRAY, not a bare object: `--format json` is array-shaped
    // everywhere in the tool (export_json, reassemble_export_pages), and the
    // decisions/steer consumer (main.rs) reads it as `arr.as_array().first()`.
    // Returning an object here would silently give that consumer zero items.
    let arr = serde_json::Value::Array(vec![root]);
...
<head>
  ...
  <title>{board_label} Board — Arrow-Kanban</title>
  <style>
{CSS}
  </style>
</head>
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Research Index — Arrow-Kanban</title>
  <style>
{CSS}
{TREE_CSS}
  </style>
...
///
/// Writes each item as `arrow-kanban/{work,research}/{type}/{id}.md`
...
///
/// Writes the item as `arrow-kanban/{work,research}/{type}/{id}/{id}.md`.
