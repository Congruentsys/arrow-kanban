...
//! Each mutation can optionally create a graph-native commit in the audit trail.
            ...
            if ci == col_idx {
                ...
                for i in 0..batch.num_rows() {
                    if i == row_idx {
                        ...
                        // caller) — so an all-empty input CLEARS the list instead of
                        // corrupting it, and "EX-1,,EX-2" drops the stray element.
                        for v in values {
                            if !v.trim().is_empty() {
...
mod tests {
    ...
    fn test_ch6742_depends_on_empty_string_clears_not_corrupts() {
        ...
        store
            .update_depends_on(&id, &["".to_string()])
            .expect("update");
        let got = store.get_list_field(&id, items_col::DEPENDS_ON).unwrap();
        assert!(
            got.is_empty(),
            "depends_on should be CLEARED by an all-empty input, got {got:?}"
        );
    }

    #[test]
    fn test_ch6742_update_list_field_drops_stray_empty_elements() {
        // DEFECT 1: a mid-list empty ("EX-1,,EX-2") must not survive as an element.
        let mut store = KanbanStore::new();
        let id = update_test_item(&mut store);
        store
            .update_depends_on(
        ...
        assert_eq!(
            got,
            vec!["EX-1".to_string(), "EX-2".to_string()],
            "empty / whitespace-only elements must be dropped"
        );
    }

    #[test]
    fn test_ch6742_remove_from_list_field_reports_whether_removed() {
        // DEFECT 2 support: the caller (the --unrelate del loop) needs to know whether the
        // flat projection actually held the edge, so a flat-ONLY edge can be reported removed
        // even with no typed relation to remove.
        let mut store = KanbanStore::new();
        let id = update_test_item(&mut store);
        store
            .update_depends_on(&id, &["EX-1".to_string(), "EX-2".to_string()])
            .expect("update");
        assert!(
            store
                .remove_from_list_field(&id, items_col::DEPENDS_ON, "EX-1")
                .unwrap(),
            "removing a present flat edge must report true"
        );
        assert!(
            !store
                .remove_from_list_field(&id, items_col::DEPENDS_ON, "EX-404")
                .unwrap(),
            "removing an absent edge must report false"
        );
        let got = store.get_list_field(&id, items_col::DEPENDS_ON).unwrap();
        assert_eq!(got, vec!["EX-2".to_string()]);
    }
