...
//! Shape-driven template generation for all 13 arrow-kanban item types.
//!
...
//!
//! Override hierarchy: project (`.arrow-kanban/shapes/`) → built-in.
...
impl ShapeLoader {
    ...
    pub fn new(root: &Path) -> Self {
        let project_dir = root.join(".arrow-kanban/shapes");
        Self {
            ...
            builtin_shapes_dir: root.join("crates/arrow-kanban/ontology/shapes"),
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_list_all_has_all_types() {
        let generator = TemplateGenerator::new(make_loader());
        let summaries = generator.list_all();
        // 13 = 7 dev (expedition, voyage, campaign, chore, hazard, signal, feature)
        // + 6 research (paper, hypothesis, experiment, measure, idea, literature).
        assert_eq!(
            summaries.len(),
            ItemType::DEV.len() + ItemType::RESEARCH.len()
        );
        assert!(
            summaries.iter().any(|s| s.item_type == ItemType::Campaign),
            "Campaign should be listed among the templatable types"
        );
    }
