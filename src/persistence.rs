...
//! Kanban persistence engine — WAL + atomic Parquet save.
//!
//! This module wraps `arrow-kanban::persist` with production-grade features:
//! - **Dirty tracking**: only save when state has changed
//! - **WAL (Write-Ahead Log)**: crash-safe atomic writes (via `parquet_store::save_named_batches`)
//! - **Graph-native commit history**: queryable audit trail via `parquet_store::CommitsTable`
//! - **Health metrics**: track save count, last save time, bytes written
//! - **Graceful shutdown**: save before exit

use crate::crud::KanbanStore;
use crate::parquet_store::{Commit, CommitsTable, persist_commits, restore_commits};
use crate::persist;
...
/// dirty tracking, atomic saves (via `parquet_store`), and graph-native commits.
...
impl PersistenceEngine {
    ...
    ///
    /// Delegates to `persist::save_all()` which uses `parquet_store::save_named_batches()`
    ...
    pub fn save(&mut self, store: &KanbanStore, relations: &RelationsStore) -> Result<SaveMetrics> {
        ...
        // Atomic Parquet save via parquet_store
...
mod tests {
    ...
    fn test_save_creates_parquet_files() {
        ...
        let data_dir = dir.path().join(".arrow-kanban");
    ...
    fn test_wal_recovery_clean() {
        ...
        std::fs::create_dir_all(dir.path().join(".arrow-kanban")).expect("mkdir");
    ...
    fn test_git_backup_persists_to_disk() {
        ...
        let commits_path = dir.path().join(".arrow-kanban/_commits.json");
