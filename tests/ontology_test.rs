...
const KB_NS: &str = "https://congruentsys.org/ns/arrow-kanban/kanban/";
...
fn installation_path_note_present() {
    let content = load_ontology();
    assert!(
        content.contains(".arrow-kanban/kanban.ttl"),
        "kanban.ttl header must document the project-local installation path (.arrow-kanban/kanban.ttl)"
    );
}
