...
//! Integration tests for arrow-kanban-server — handler dispatch without NATS.
//!
...
fn test_unpersistable_mutation_is_reported_as_an_error_not_acked() {
    ...
    // The refusal must NAME the server session — the redo-or-not rule
    ...
    assert!(
        ...
        "the refusal must carry the server session identity: {resp}"
    );
}
...
fn test_server_degrades_and_refuses_subsequent_mutations() {
    ...
    // The degraded-gate refusal is the message an operator sees repeatedly
    // mid-outage — it must carry the same session comparand as the first
    // failure, or redo-or-not is only checkable against the message that
    // scrolled away.
    assert!(
        second["error"]
            .as_str()
            .unwrap_or("")
            .contains("session start_ms="),
        "the degraded refusal must carry the server session identity: {second}"
    );
}
...
fn test_gate_recovers_and_mutations_resume() {
    ...
    state.health = arrow_kanban_server::health::HealthGate::with_probe(
        1024,
