...
fn a_refused_write_never_becomes_readable() {
    ...
    // a per-write fsync without the guarantee. Full clone-persist-swap rollback WOULD close it
    // but is disproportionate for a one-item, reader-only, restart-resolved window.
    let after_restart = fresh_state(root).store.active_item_count();
    assert_eq!(
        after_restart, before,
        "the pre-gate leaked write must NOT survive a restart — only acked writes are durable \
         (got {after_restart}, expected {before}); the ≤1 in-memory window is transient by design"
    );
}
...
fn an_interrupted_save_is_never_silently_superseded() {
    use arrow_kanban::persistence::PersistenceEngine;
