...
///
/// These are not the same path — the store is `<root>/.arrow-kanban` — and
...
pub fn store_dir(root: &Path) -> PathBuf {
    arrow_kanban::persist::data_dir(root).unwrap_or_else(|_| root.to_path_buf())
}
...
impl HealthGate {
    ...
    pub fn admit_mutation(&mut self, data_dir: &Path) -> Result<(), String> {
        if !self.is_degraded() {
            ...
            // real save is the authoritative test; its failure flips the gate (via
            // `record_persist_failure`) and refuses everything after. The one pre-gate write is
            // bounded, reader-only, never acked to the writer, and dropped on restart (see the
            // `a_refused_write_never_becomes_readable` acceptance test).
            return Ok(());
        }
